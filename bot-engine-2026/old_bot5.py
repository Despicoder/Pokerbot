...
Polarized EV-Maximizer + Anti-Shove Architecture
"""
...
class Player(BaseBot):
    ...
    def _auction_bid(self, state):
        ...
        if eq > 0.80:
            ...
            # Value bid, but occasionally block static 60-chip bidders
            bid = int(pot * 0.25)
            if random.random() < 0.25:
                bid = max(int(pot * 0.30), random.randint(61, 65))
        else:
            # Mix between 0 (avoiding tax), noise bids, and blocking bids
            r = random.random()
            if r < 0.15: 
                # Block cheap info requests from bots like AllInOrFold
                bid = max(int(pot * 0.20), random.randint(61, 65))
            elif r < 0.40:
                # Noise bid to mask weakness
    ...
    def _heuristic(self, state, equity):
        ...
        # --- DEACTIVATE OPPONENT MODEL ON SHOVES & BLOATED POTS ---
        if cost >= pot * 0.95:
            # Facing an overbet/shove. They aren't bluffing for their whole stack. Play extremely tight.
            val_thresh  += 0.15
            call_thresh += 0.15
        elif pot >= 1000:
            # Pot is massive. Do not bluff catch. Ignore the model.
            val_thresh  += 0.15
            call_thresh += 0.10
        else:
            # Normal pot: use the Bayesian model to exploit passive/aggressive tendencies
        ...
        # EXTREME LOSS MINIMIZATION SCALING
        if pot > 400:
        ...
        # Never raise more than twice per street unless we have the nuts, and NEVER raise in massive pots without the absolute nuts.
        if pot > 400:
            can_raise_more = (equity > 0.95)
        else:
