...
Polarized EV-Maximizer + System 2 Deep Think Architecture
"""
...
class Player(BaseBot):
    ...
    def get_move(self, game_info, state):
        ...
        # Fast "System 1" equity for standard decisions
        ...
        if action is None: 
            action = self._heuristic(state, equity)

        # 3. SYSTEM 2: DEEP THINK & GLOBAL OVERRIDE
        # This executes AFTER the blueprint to catch CFR hallucinations and force tight play against shoves
        board = list(state.board)
        cost  = state.cost_to_call
        pot   = max(state.pot, 1)

        def safe_passive():
            if state.can_act(ActionCall) and cost > 0: return ActionCall()
            if state.can_act(ActionCheck): return ActionCheck()
            return ActionFold()

        # Trigger high-accuracy 250-sample MC if stakes are extremely high post-flop
        if board and (cost > 400 or (isinstance(action, ActionRaise) and pot > 400)):
            equity = _equity_mc(list(state.my_hand), board, opp_known=self._opp_revealed or None, n=250)

        # Intercept and clamp aggressive actions (Raises)
        if isinstance(action, ActionRaise):
            # Evaluate Absolute Hand Strength (0=High Card, 1=Pair, 2=Two Pair, 3=Trips, 4=Straight...)
            my_rank = _evaluate_best(list(state.my_hand), board)._t[0] if board else 1
            deny_raise = False
            
            # Rule 1: Never re-raise when our card is exposed
            if self._our_card_exposed is not None and cost > 0:
                deny_raise = True
            # Rule 2: Do not inflate massive pots without a guaranteed premium hand AND highly accurate equity
            elif pot > 400 and (my_rank < 2 or equity < 0.85):
                deny_raise = True
            # Rule 3: Hard cap on raise wars
            elif self._raises >= 2 and (my_rank < 3 or equity < 0.90):
                deny_raise = True

            if deny_raise:
                action = safe_passive()

        # Intercept suicidal calls (Shove Defense)
        if isinstance(action, ActionCall) and cost > 400 and board:
            # If they overbet/shove and our true, deep-think equity is less than 70%, fold out of the trap.
            if equity < 0.70:
                action = ActionFold()
    ...
    def _auction_bid(self, state):
        ...
        # POLARIZED BIDDING + ACTIVE AUCTION CONTESTING
        if eq > 0.80:
            bid = int(pot * (eq - 0.5) * 2.5) 
        elif eq > 0.60:
            # Value/Blocking bid
            bid = int(pot * random.uniform(0.20, 0.50))
        else:
            # Mix 0, small noise, and occasional large blockers to spite static 60-chip bidders
            r = random.random()
            if r < 0.20:
                bid = int(pot * random.uniform(0.30, 0.60)) 
            elif r < 0.40:
    ...
    def _blueprint(self, state, equity):
        ...
        return _label_to_action(label, state, pot, state.opp_chips, ActionFold, ActionCall, ActionCheck, ActionRaise)
    ...
    def _heuristic(self, state, equity):
        ...
        def _raise(frac):
            ...
            return ActionRaise(max(mn, min(int(pot*frac), mx)))
